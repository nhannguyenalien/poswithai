...
CREATE OR REPLACE FUNCTION decrement_stock_or_fail(
  p_variant_id TEXT,
  p_tenant_id TEXT,
  p_quantity INTEGER,
  p_updated_at TEXT
...
BEGIN
  ...
  SET qty = qty - p_quantity, updated_at = p_updated_at
  WHERE product_variant_id = p_variant_id
    AND tenant_id = p_tenant_id
    AND qty >= p_quantity
  RETURNING qty INTO remaining_qty;

  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'INSUFFICIENT_STOCK:' || p_variant_id;
  END IF;
...
BEGIN
  ...
  ELSE
    ...
    END IF;
    IF already_paid + p_amount < target_order.total THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'REFUND_EXCEEDS_REMAINING';
    END IF;
  END IF;
